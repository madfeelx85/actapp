...
async def generate_page(request: Request, db: AsyncSession = Depends(get_db_session)):
    ...
    return templates.TemplateResponse("acts_generate_page.html", {"request": request, "acts": acts})
...
async def generate_act_file(act_id: int, db: AsyncSession = Depends(get_db_session)):
    ...
    result = await db.execute(select(BuildObject).where(BuildObject.id == act.build_object_id))
