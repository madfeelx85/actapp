...
async def create_object(obj_in: BuildObjectCreate, db: AsyncSession = Depends(get_db_session)):
...
async def update_object(object_id: int, update_in: BuildObjectUpdate, db: AsyncSession = Depends(get_db_session)):
    obj = await crud.get(db, object_id)
    if not obj:
        raise HTTPException(status_code=404, detail="Object not found")
