...
async def update_act(
    act_id: int, update_in: ActUpdate, db: AsyncSession = Depends(get_db_session)
):
    act = await crud.get(db, act_id)
    if not act:
        raise HTTPException(404, detail="Act not found")
...
async def get_acts_by_object(
    build_object_id: int, db: AsyncSession = Depends(get_db_session)
):
